...
def main():
    ...
    # Filter out processed ARKs
    remaining_arks = [ark for ark in all_arks if ark not in processed_arks]
    
    if not remaining_arks:
        print("All ARKs already processed.")
        return

    # Process in a smaller batch (e.g., 5 ARKs)
    batch_size = 1
    batch = remaining_arks[:batch_size]
    print(f"Processing batch of {len(batch)} ARKs...")
    ...
    print(f"Batch complete. {len(remaining_arks) - len(batch)} ARKs remaining.")
