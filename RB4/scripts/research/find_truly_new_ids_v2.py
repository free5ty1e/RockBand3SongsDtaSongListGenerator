...
def main():
                    ...
                    with mmap.mmap(f.fileno(), 0, access=mmap.ACCESS_READ) as mm:
                        ...
                        while pos != -1:
                            ...
                            for m in matches:
                                ...
                                # Logic for deciding if it's a "new" song or a "crowd request" version
                                is_request = s.startswith('request_')
                                
                                if s not in known and s.lower() not in blacklist:
                                    if '_' in s and not s.startswith('_'):
                                        # If it's a request, we always keep it (as requested by user)
                                        # If it's not a request, we only keep it if it's truly new
                                        if is_request or (not is_request):
                                            results[s] = ark_path
    ...
    with open('/workspace/RB4/output/truly_new_candidates_with_paths.txt', 'w') as f:
        for s in sorted(results.keys()):
            f.write(f"{s},{results[s]}\n")
