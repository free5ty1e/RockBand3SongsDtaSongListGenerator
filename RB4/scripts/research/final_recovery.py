...
def recover_meta(ark_path, target_ids):
    found = {}
    try:
        with open(ark_path, 'rb') as f:
            content = f.read()
            for tid in target_ids:
                pos = content.find(tid.encode('ascii'))
                if pos != -1:
                    start = max(0, pos - 512)
...
def main():
    ...
    for d in ark_dirs:
        ...
        for ark in arks:
            print(f"Processing {os.path.basename(ark)}...")
            meta = recover_meta(ark, target_ids)
    ...
    print(f"Recovered metadata for {len(all_results)} IDs")
