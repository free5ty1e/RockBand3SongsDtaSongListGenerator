...
def load_all_known_ids():
    ...
    # 3. From the final sorted list
    sorted_list_path = '/workspace/RB4/output/SongListSortedByArtist.txt'
    if os.path.exists(sorted_list_path):
        with open(sorted_list_path, 'r') as f:
            for line in f:
                # The sorted list usually has "Artist - Title"
                if ' - ' in line:
                    parts = line.split(' - ')
                    title = parts[1].lower()
                    slug = re.sub(r'[^a-z0-9]', '', title)
...
def main():
    known = load_all_known_ids()
    print(f"Loaded {len(known)} known identifiers.")
                    ...
                    with mmap.mmap(f.fileno(), 0, access=mmap.ACCESS_READ) as mm:
                        ...
                        while pos != -1:
                            ...
                            for m in matches:
                                ...
                                # CRITICAL FILTER: must be truly new
                                if s not in known and s.lower() not in blacklist:
                                    if '_' in s and not s.startswith('_'):
                                        # Avoid crowd request patterns if we already have the song
                                        if s.startswith('request_'):
                                            song_id = s.replace('request_', '')
                                            if song_id in known:
                                                continue
                                        truly_new_candidates.add(s)
                            pos = mm.find(b"RBSongMetadata", pos + 1)
            except Exception as e:
                print(f"Error scanning {ark_path}: {e}")
    
    print(f"\nFound {len(truly_new_candidates)} truly new candidates")
    with open('/workspace/RB4/output/truly_new_candidates.txt', 'w') as f:
        for s in sorted(list(truly_new_candidates)):
            f.write(f"{s}\n")
